...
(define (draw)
	...
	(define render-width 300)
	(define render-height 300)
	...
	(define screen-min-x 100)
	(define screen-max-x 300)
	(define screen-min-y 100)
	(define screen-max-y 300)
	...
	(define x-steps-per-pixel 2)
	(define y-steps-per-pixel 2)
	(define rays-per-pixel (* x-steps-per-pixel y-steps-per-pixel))
	
	(define camera '(200 200 -80))
	...
	(define light1 (list '(0 0 -30) '(0.5 0.5 0.5) '(0.8 0.8 0.8)))
	...
	(define (floor-color x z)
		...
		; 	'(0 0 0)
		; )
		; x and z offset to get away from (0, 0) distortion
	...
	(define (ray-color origin direction depth)
		(if (> depth MAX_RAY_RECURSION_DEPTH) (calc-bg-ray direction) (begin
			...
			(define sphere (cadr closest-intersect))
			(if (and sphere t)
				(begin
					...
					(define (compute-lighted-color lights current-color)
						(if (null? lights) current-color (begin
							...
							(if (sphere-in-shadow intersect-point light spheres-except-current)
								...
								(compute-lighted-color (cdr lights) current-color)
								; not in shadow
								(begin
									...
									(define diffuse-color
										(if (< diffuse-comp 0) '(0 0 0)
											...
											(mul-vec (elwise-mul-vec (cadr light) (cadr sphere-mat)) diffuse-comp)
										)
									)
									...
									(define specular-color 
										(if (< view-dot-reflectance 0) '(0 0 0)
											...
											(mul-vec (elwise-mul-vec (caddr light) (caddr sphere-mat))
												; raise view-dot-reflectance to power of shininess constant (4th elemnt in material list)
												(expt view-dot-reflectance (cadr (cddr sphere-mat)))
											)
										)
									)
					...
					(define color-no-ambient (compute-lighted-color lights '(0 0 0)))
					...
					(define reflected-color (if reflectivity-color
						(begin
							; Also, flip ray so its going from intersection point to origin
							(define normalized-ray (mul-vec (normalize-vec direction) -1))
							(define scaled-normal (mul-vec normal (* 2 (dot-vec normal normalized-ray))))
							(define reflected-ray (sub-vec scaled-normal normalized-ray))
							(elwise-mul-vec reflectivity-color (ray-color intersect-point reflected-ray (+ depth 1)))
						)
						'(0 0 0)
					))
					; Clamp color components to range [0, 1]
					(define final-color-unclamped  (add-vec (add-vec color-no-ambient ambient-color) reflected-color))
				...
				(begin
					...
					(if (= (car (cdr direction)) 0)
						...
						(begin
							...
							(if (and (> t 0.01) (> intersect-x 50) (< intersect-x 350) (> intersect-z 50) (< intersect-z 250))
								; intersects floor
								; (begin
								; 	(print t origin direction)
									(floor-color intersect-x intersect-z)
								; )
								(calc-bg-ray direction)
							)
						)
